...
KDA <- function(physeq, grouping_column, analyse="abundance", method="dscore",
  ...
  Sets<-pearson.group(as.matrix(analyse.data),corr)
  ...
  for(i in unique(Sets)){
    ...
    if(method=="dscore"){
      ks=dscore(x,meta_table$Groups,lower=1,upper=10,m=3)
    }
    if(method=="sscore"){
      ks=sscore(x,meta_table$Groups,lower=10^-3,upper=10^3,m=10)
    }
  ...
  for(i in kscore_selected){
    tmp<-melt(as.matrix(analyse.data[Sets==i,,drop=F]))
    ...
    tmp$Set<-paste("Set",tmp$Set,sprintf("Adj.p = %0.5g",KMtable[as.character(tmp$Set),"kscore.adjusted"]),
                   cut(KMtable[as.character(tmp$Set),"kscore.adjusted"],breaks=c(-Inf, 0.001, 0.01, adjusted.p.value.threshold, Inf),label=c("***", "**", "*", "")))
...
plot_kda <- function(df){
  p<-ggplot(aes(x=Set_variable,y=Value,color=Groups),data=df)+theme_bw()+geom_boxplot(outlier.size = NA)+facet_grid(. ~ Set, drop=TRUE,scales="free",space="free_x")
  p<-p+theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))+theme(strip.text.x = element_text(size = 12, colour = "black", angle = 90,vjust = 0))
  p<-p+theme(strip.background = element_rect(fill = "white"))+ylab("")+xlab("")
  return(p)
}
