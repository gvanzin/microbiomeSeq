...
# `physeq` is a required phyloseq object containing taxa abundance and meta data.  `
# grouping_column` is the variable in the meta data with respect to which the data should be grouped, 
# `pvalueCutoff` the threshold p-value in `anova` of distance matrices, default set to `0.05`. 
# `env.variables` is a list of variables prefered to be on the cca plot.
# `exclude.variables` a list of variables to be excluded from the cca plot. 
# `num.env.variables` is an integer specifying the number of variables to show on the cca plot.
# require(vegan)
# require(gridBase)
# require(ggplot2)
plot_cca <- function(physeq, grouping_column, pvalueCutoff=0.01,norm_method=NULL, env.variables=NULL, 
                     num.env.variables=NULL, exclude.variables="Country", draw_species=F){
  ...
  #Draw sites
  p<-ggplot()
  p<-p+geom_point(data=df_sites,aes(x,y,colour=Groups))
  #Draw biplots
