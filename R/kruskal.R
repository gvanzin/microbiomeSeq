...
#' data(pitlatrine)
#' physeq <- pitlatrine
#' kw_sig <-  kruskal_expression(physeq, "Country")
...
kruskal_abundance <- function(physeq, grouping_column,pvalue.threshold=0.05)
{
  ...
  for(i in sig_res){
    ...
    rank_label <- paste(paste(rep("⬛", rank),collapse=""),max(df_accuracy$rank)+1-rank)
    tmp<-data.frame(data[,i],meta_table$Groups, rep(paste(rank_label)), rep(paste(i," p.adj = ",sprintf("%.5g",kruskal.wallis.table[kruskal.wallis.table$id==i,"q.value"]),sep=""),dim(data)[1]))
