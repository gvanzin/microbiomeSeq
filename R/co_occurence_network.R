...
#' data(pitlatrine)
#' physeq <- pitlatrine
#' co_network <- co_occurence_network(physeq,grouping_column = "Country")
#' 
...
network_correlation <- function(physeq, grouping_column, select.condition, method, filename){
  ...
  # === rarefy
  comm.data<-rrarefy(abund_table,min(rowSums(abund_table)))
  ...
  for(a in 1:length(trts)){
    ...
    if(method=="bicor"){
      ...
      new.row<-data.frame(rep(trts[a],dim(temp)[2]),melt(as.matrix(res$bicor)),melt(as.matrix(res$p))[,3])
      
    } else if (method=="cor"){
      #Pearson correlation
      res<-corAndPvalue(temp,use="p")
      ...
      new.row<-data.frame(rep(trts[a],dim(temp)[2]),melt(as.matrix(res$cor)),melt(as.matrix(res$p))[,3])
    }
