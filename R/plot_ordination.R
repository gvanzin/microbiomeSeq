...
#' 
plot_ordination <- function(ordination.res, method, pvalue.cutoff=0.05, show.pvalues=T, N=5,
  ...
  p<-ggplot(data=ord_res,aes(x,y,colour=Groups))
  p<-p + geom_point(alpha=0.5,size = 2)
  p<-p+theme_bw()
  p<-p+ annotate("text",x=ord_res.mean$x,y=ord_res.mean$y,label=ord_res.mean$group,size=6,colour=cols,family="Courier",fontface="bold",alpha=0.8,vjust=0.3)
  ...
  if(method=="NMDS"){
    ...
    p <- p + annotation_custom(grob = textGrob(label = stress.label, hjust = 0, gp = gpar(cex = 1.5,fontsize=8)),
  ...
  if(!is.null(gt) && !is.null(anova_table)){
    out <- grid.arrange(gt,anova_table,heights = unit.c(unit(1, "null"), th))
  }
  else if(is.null(gt) && !is.null(anova_table)){
    out <- grid.arrange(p,anova_table,heights = unit.c(unit(1, "null"), th))
  }
  return(out)
}
...
plot_betadisper <- function(betadisper_res, show.pvalues=T, pvalue.cutoff, N){
  ...
  anova_table <- tableGrob(anova_table_display,rows = NULL,cols = NULL,theme =ttheme_minimal()) #set rows/cols to null to get rid of the row numbering
  title <- textGrob("BETA-DISPERSION",gp=gpar(fontsize=12))
  anova_table <- gtable_add_rows( anova_table, heights = grobHeight(title) + unit(5,"mm"), pos = 0)
  anova_table <- gtable_add_grob( anova_table,  title, 1, 1, 1, ncol(anova_table))
...
plot_ordisurf <- function(sol, meta_table, env.variable, grouping_column){
  ...
  ordi<-ordisurf(sol,meta_table$var ,plot = FALSE, bs="ds")
  ...
  #make the plot
  p<-ggplot()+stat_contour(data = ordi.mite.na, aes(x = x, y = y, z = z, colour = ..level..),positon="identity") #can change the binwidth depending on how many contours you want
  p<-p+geom_point(data=df,aes(x,y,fill=Groups),pch=21,size=3)
  ...
  p<-p+labs(colour = paste(env.variable)) #another way to set the labels, in this case, for the colour legend
  p<-p+theme_bw()
  return(p)
}
